...
const SaleAnnouncements = () => {
  ...
  useEffect(() => {
    const fetchSaleAnnouncements = async () => {
      try {
        const response = await fetch(`http://localhost:3000/announcements?forSale=true`);
        const data = await response.json();
        setSaleAnnouncements(data);
      } catch (error) {
        console.error('Error fetching announcements:', error);
      }
    };

    fetchSaleAnnouncements();
  }, []);
...
useEffect(() => {
  ...
  switch (sortCriteria) {
    ...
    case 'low-to-high':
      ...
      break;
    default:
      sorted = saleAnnouncements;
  }
  
  setSortedAnnouncements(sorted);
}, [saleAnnouncements, sortCriteria]);
...
export default SaleAnnouncements;
