...
const RentAnnouncements = () => {
  const [rentAnnouncements, setRentAnnouncements] = useState([]);
  ...
  useEffect(() => {
    const fetchRentAnnouncements = async () => {
      try {
        const response = await fetch(`http://localhost:3000/announcements?rent=true`);
        const data = await response.json();
        setRentAnnouncements(data);
      } catch (error) {
        console.error('Error fetching announcements:', error);
      }
    };

    fetchRentAnnouncements();
  }, []);
...
useEffect(() => {
  ...
  switch (sortCriteria) {
    ...
    case 'low-to-high':
      ...
      break;
    default:
      sorted = rentAnnouncements;
  }
  
  setSortedAnnouncements(sorted);
}, [rentAnnouncements, sortCriteria]);
...
export default RentAnnouncements;
